...
### (1) 정규화 선형회귀
#### 1. 과적합과 과소적합
* 과적합(과대적합): 모델이 학습 데이터를 과하게 학습하는 것
* 데이터 과적합시: 일반화 성능 낮아짐-> 이미 학습한 훈련용 데이터에 대한 성능은 높게 나오지만, 아직 학습하지 않은 테스트 데이터에 대한 성능은 낮게 나옴
  * 모델이 학습 데이터에 너무 과하게 맞추어지면 새로운 데이터에 일반화하기가 어려움
* 과소적합: 모델이 너무 단순해서 학습 데이터조차 제대로 에측하지 못하는 경우

#### 2. 정규화 선형회귀
* 회귀분석에 과적합되면 계수의 크기가 되도하게 증가: 이를 방지하기 위해 계수의 크기를 제한하는 방법 -> 정규화 선형회귀
* 릿지(Ridge), 라쏘(Lasso), 엘라스틱넷(Elastic Net)

#### 3. 정규화 선형회귀의 종류
* 라쏘
  * L1규제, 가중치들의 절댓값의 합을 최소화하는 것을 제약조건으로 추가
  * 일정한 상숫값이 패널티로 부여-> 일부 불필요한 가중치 파라미터를 0으로 만들어 분석에서 제외
  * 몇 개의 의미 있는 변수만 분석에 포함
 
* 릿지
  * L2규제, 가중치들의 제곱합을 최소화하는 것을 제약조건으로 추가
  * 일부 가중치 파라미터 제한하지만, 완전히 0으로 만들지 않고 0에 수렴하도록 만듦
  * 매우 크거나 작은 이상치의 가중치를 0에 가깝게 유도-> 선형 모델의 일반화 성능을 개선하는 데 사용
 
* 엘라스틱넷
  * 라쏘와 릿지를 결합한 모델
  * 가중치의 절댓값과 제곱합을 동시에 제약조건으로 가지는 모델
 
### (2) 일반화 선형회귀
#### 1. 일반화 선형회귀의 개념
* 회귀분석은 종속변수가 정규분포를 따른다는 정규성을 전제로 함
* 일반화 선형회귀: 종속변수가 범주형 자료거나 정규성을 만족하지 못하는 경우 종속변수를 적절한 함수 $f(x)$로 정의한 다음, 이 함수와 독립변수를 선형결합하여 회귀분석을 수행

#### 2. 일반화 선형회귀의 요소
* 확률 요소(Random Component): 종속변수의 확률분포를 규정하는 성분
* 선형 예측자(Linear Predictor, 체계적 성분): 종속변수의 기댓값을 정의하는 독립변수들 간의 선형결합
* 연결함수(Link Function): 확률 요소와 선형 예측자를 연결하는 함수

#### 3. 일반화 선형회귀의 종류
* 로지스틱 회귀
  * 종속변수가 범주형 변수(0또는 1)인 경우
  * 종속변수와 독립변수 간의 관계를 나타내어 예측 모델을 생성한다는 점에서 선형 회귀 분석과 동일
  * 그러나 독립변수에 의해 종속변수의 범주로 분류한다는 측면은 '분류 분석'방법으로 분류
 
* 포아송 회귀
  * 종속변수가 특정 시간동안 발생한 사건의 건수에 대한 도수 자료(count data, 음수가 아닌 정수)인 경우이면서, 종속변수가 정규분포를 따르지 않거나 등분산성을 만족하지 못하는 경우 사용
  * 선형회귀모형이 최소제곱법으로 모수를 추정한다면 포아송 회귀모형은 최대 가능도 추정(MLF, Maximum Likelihood Estimation)을 통해 모수 추정
 
### (3) 더빈 왓슨(Durbin Watson) 검정
#### 1. 오차항의 상관관계
* 오차항이 상관관계를 갖는 경우: 대부분 시계열 데이터의 경우
* 시계열 데이터: 시간의 흐름대로 나열된 데이터
  * 시계열 데이터는 연속적인 일련의 관측치들이 서로 상관됨
  * 즉, 하나의 잔차항의 크기가 이웃하는 다른 잔차항의 크기와 서로 일정한 관련 있음

#### 2. 더빈 왓슨 검정
* 회귀분석에 있어서는 오차항이 서로 연관성이 없어야 함: 회귀분석에서의 오차항의 공분산은 0
* 만약 자기상관성이 있다면 회귀분석이 아니라 시계열 분석이나 다른 분석 방법 수행해야 함
* 더빈 왓슨 검정: 회귀분석에 있어 이러한 자기상관성이 존재하는지 검정하는 방법
* '더빈 왓슨 검정' 통계량 값이 2에 가까울수록 오차항의 자기상관이 없다는 의미
* 0에 가깝다면 0의 상관관계, 4에 가깝다면 음의 상관관계

![image](https://github.com/qlkdkd/2-winter/assets/71871927/2bd3550b-f27e-4dbd-91cf-b396769d6d0f)
