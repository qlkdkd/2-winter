...
### (2) 가설 검정
1. 가설검정의 개념
  * 통계적 가설검정(statistical hypothesis test): 모집단의 특성에 대한 주장 또는 가설을 세우고 표본에서 얻은 정보를 이용해 가설이 옳은지를 판정하는 과정
  * 귀무가설, 대립가설

2. 귀무가설
  * 모집단이 어떠한 특징을 지닐 것으로 여겨지는 가설, 일반적으로 '차이가 없다', '같다(=)'기호 사용하여 나타낼 수 있는 가설, $H_0$으로 나타냄
  * 실험, 연구를 통해 기각하고자 하는 어떤 가설, 대립가설과 상반, 귀무가설의 기각을 통해 입증하고자 하는 주장을 관찰할 수 있음

3. 대립가설
  * 귀무가설에 반대되는 가설, 귀무가설이 틀렸다고 판단될 경우 채택되는 가설
  * $H_1$로 표기
  * 실험, 연구를 통해 증명하고자 하는 새로운 아이디어 혹은 가설에 해당

4. 제 1종오류와 제 2종오류
  * 제 1종오류: 귀무가설이 사실인데 귀무가설이 틀렸다고 결정하는 오류
  * 제 2종오류: 귀무가설이 사실이 아님에도 불구하고 귀무가설이 옳다고 결정하는 경우
...
6. 기각역
  ...
  * 검정통계량이 기각역 내에 있으면 귀무가설 기각, 기각역 외의 채택역에 있으면 귀무가설 기각 불가능
  * 임계값: 기각역의 경곗값

7. 유의 수준
  * 유의 수준: 귀무가설이 참인데도 이를 잘못 기각하는 오류를 범할 확률의 최대 허용 한계
  * 1%와 5%를 자주 사용, 가설검정을 수행하는 환경에 맞게 조절 가능
  * 가설검정을 수행하는데 제1종 오류와 제2종 오류 모두 줄이는 것이 이상적
     * 그러나 제1종 오류와 제2종 오류는 반비례관계-> 제1종오류를 허용할 수 있는 최대 확률 유의수준($\alpha$)을 설정하여 가설검정 수행

8. 유의 확률
  * 유의확률(p-value): 귀무가설을 지지하는 정도
  * p-value가 유의수준 $\alpha$보다 작은 경우에는 귀무가설이 참임을 가정했을 때 이러한 결과가 매우 적다고 해석할 수 있음
     * -> 귀무가설 기각, 대립가설 채택

### (3) 비모수검정
1. 모수검정 vs 비모수검정
  * 모수검정: 표본이 정규성을 갖는다는 모수적 특징을 이용하는 통계 방법 -> 표본의 정규성 반드시 확보
  * 비모수검정
    * 정규성 검정에서 정규분포를 따르지 않는다고 증명되거나 표본 군집 당 10명 미만의 소규모 실험에서와 같이 정규 분포임을 가정할 수 없는 경우에 사용
    * 정규분포를 따르지 않는 경우 자료를 크기 순으로 배열하여 순위를 매긴 다음 순위의 합을 통해 차이를 비교하는 순위합검정 적용 가능  -> 모수의 방법 사용 X(비모수검정)
    * 모수의 분포에 대해 어떠한 가정(정규분포)도 하지 않는 검정
  * 숫자로 표현되지만 수량화될 수 없고 평균을 낼 수도 없는 서열척도의 경우: 연속형 자료는 아니지만, 순위의 합을 이용하는 비모수적 방법 적용하는 것은 가능
  * 이상치로 인해 평균보다 중앙값이 더 바람직한 경우, 표본의 크기가 작은 경우, 순위와 같은 서수 데이터인 경우에 사용한다.

2. 모수 검정과 비모수 검정의 차이

모수 검정(parameteric test)|비모수검정(nonparametric test)
---|---
등간척도, 비율척도|명목척도, 서열척도
평균|중앙값
피어슨 상관계수|스피어만 순위상관계수
one sample t-test, two sample t-test, paired t-test, one way anova|부호검정, Wilcoxon 부호순위검정, Mann-Whitney 검정, Kruskal Wallis 검정
